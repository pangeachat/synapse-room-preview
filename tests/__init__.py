...
from synapse_room_preview import SynapseRoomPreview
...
def create_module() -> SynapseRoomPreview:
    # Create a mock based on the ModuleApi spec, but override some mocked functions
    # because some capabilities are needed for running the tests.
    module_api = Mock(spec=ModuleApi)

    # If necessary, give parse_config some configuration to parse.
    config = SynapseRoomPreview.parse_config({})

    return SynapseRoomPreview(config, module_api)
